...
#' @param formula  Right-sided formula indicating the independent
#'                 variable to plot on the x axis (e.g. ~x).
#'                 If not specified, the first column is
#'                 assumed to contain this data.
#' @param subset   Logical (condition) specifying which data to
...
#' @param backfun  Function applied to the data before plotting. This
#'                 is useful to back-transform data to the measurement scale.
#' @param xfun     Transformation applied to the independent variable before
...
#'                 (NULL = not plotted)
#' @param line.col Color of line that connects predicted values
#'                 (NULL = not plotted)
#' @param line.lty Type of line connecting predicted values
#' @param line.lwd Width of line connecting predicted values
#' @param ci.col   Color for line showing confidence intervals
                           ...
                           ci.f=1)
{
    ...
    ## subset data
    m <- match(c("subset","formula"),names(mf),0)
    if(m[1] > 0) {
        idx <- eval( mf[[ m[1] ]],
                    envir = data,
                    enclos = parent.frame())
