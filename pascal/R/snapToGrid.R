...
#'
#' @param which specify which results to return. Can be either
#'     "points" or "all".
#'
...
snapToGrid <- function(d, theta = 0, sz = 1, which = "points") {
    ...
    m <- rotate2D(r$par[1]) %*%
        translate2D(r$par[2], r$par[3]) %*%
        scale2D(r$par[4], r$par[4])

    if (which == "points") {
        apply2D(m, x = d)
    } else {
        list(
            points = apply2D(m, x = d),
            params = list(
                theta = r$par[1],
                xshift = r$par[2],
                yshift = r$par[3],
